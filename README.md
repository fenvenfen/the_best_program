...
Cинхронне, так і асинхронне виконання коду
Як JavaScript працює з асинхроним кодом
Event Loop в JavaScript розібратися як працює
    Загальний алгоритм роботи двигуна:
    Поки є завдання:
    виконувати їх, починаючи з найстарішого завдання.
    ...
    Завдання встановлюються – механізм їх обробляє – потім чекає на інші завдання (під час сну та споживаючи близько нуля ЦП).

    Може статися так, що завдання приходить, коли движок зайнятий, а потім воно ставиться в чергу.
    Завдання утворюють чергу, так звану «чергу макрозадач» (термін v8):
    

Що таке callbacks і promises, в чому є проблема callbacks і як її вирішили.
Що таке взагалі апі і для чого потрібне 

        SOAP API 
      Ці API використовують простий протокол доступу до об’єктів. Клієнт і сервер обмінюються повідомленнями за допомогою XML. Це менш гнучкий API, який був більш популярним у минулому.

      API RPC
      Ці API називаються віддаленими викликами процедур. Клієнт виконує функцію (або процедуру) на сервері, а сервер надсилає результат назад клієнту.
      ...
      REST API
      Це найпопулярніші та найгнучкіші API, які сьогодні є в Інтернеті. Клієнт надсилає запити на сервер як дані. Сервер використовує цей вхід клієнта для запуску внутрішніх функцій і повертає вихідні дані назад клієнту. Нижче розглянемо REST API докладніше.


      Розібратися яким чином потрібно працювати з API в angular

          Більшість інтерфейсних програм потребують зв’язку із сервером через протокол HTTP, щоб завантажувати або завантажувати дані та отримувати доступ до інших внутрішніх служб. Angular надає клієнтський HTTP API для програм Angular, HttpClientклас обслуговування в .@angular/common/http

            Клієнтська служба HTTP пропонує такі основні функції.

            Можливість запитувати введені об’єкти відповіді
            Спрощена обробка помилок
            Характеристики тестованості
            Перехоплення запитів і відповідей

            Використовуйте HttpClient.get()метод для отримання даних із сервера. Асинхронний метод надсилає HTTP-запит і повертає Observable, який видає запитані дані, коли отримано відповідь. Тип повернення залежить від значень observeі responseType, які ви передаєте виклику.

            Метод get()приймає два аргументи; URL-адресу кінцевої точки, з якої потрібно отримати, і об’єкт параметрів , який використовується для налаштування запиту.
      ...
      HttpClientModule за що відповідає цей модуль
      HttpClient як правильно використовувати і що цей класс включає
      Що таке interceptors а саме HttpInterceptor які можливості він має
